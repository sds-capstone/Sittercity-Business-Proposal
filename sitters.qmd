...
rating_sitters_model <- rating_sitters_wide %>% 
  group_by(sitters_average_rating)

sitters_rating_model<- lm(mean_pay_rate_sitters ~ sitters_average_rating, data= rating_sitters_model)

get_regression_table(sitters_rating_model)
summary(aov(sitters_rating_model))


sitters_rating_model_plot <- ggplot(rating_sitters_model,
       aes(x=sitters_average_rating,
           y=mean_pay_rate_sitters)) +
  geom_point() +
  geom_smooth(se=FALSE, method = lm) + labs(title = "sitters_rating_model")
  
sitters_rating_model_plot 
```

```{r}
#Interaction model: 
sitters_age_model<- lm(mean_pay_rate_sitters ~ sitters_, data= rating_sitters_wide)

get_regression_table(sitters_rating_model)
summary(aov(sitters_rating_model))


sitters_rating_model_plot <- ggplot(rating_sitters_wide,
       aes(x=sitters_average_rating,
           y=mean_pay_rate_sitters)) +
  geom_point() +
  geom_smooth(se=FALSE, method = lm) + labs(title = "sitters_rating_model")
